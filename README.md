...
| `remember_memory` | 将重要信息主动写入长期记忆，供后续对话检索使用 | 无 |
...
- **主动写入**：Agent 通过 `remember_memory` 工具决定何时写入，只在用户明确要求记住、或检测到明显的用户特质/偏好/事实时触发，避免噪音堆积
- **记忆类别**：`explicit`（用户明确要求）、`trait`（推断的偏好/习惯）、`fact`（客观信息）、`summary`（合并压缩后生成）
- **记忆合并**：`MemoryConsolidator` 在后台周期性检查，当记忆数量超过阈值（默认 50 条）时，用 LLM 对全部记忆做合并、去重、压缩，替换原始条目，防止长期膨胀
- 支持两种检索模式：
